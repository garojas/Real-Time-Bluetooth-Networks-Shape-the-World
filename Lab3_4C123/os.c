...
struct tcb{
  ...
  struct tcb *next;  // linked-list pointer
   ...
   // nonzero if this thread is sleeping
			int32_t *blocked;  // pointer to semaphore determining if blocked or not
			uint32_t sleep;    // sleep timer
...
// ****IMPLEMENT THIS****
		...
		eventPeriod[event_index].Periodic_TaskPt = thread;
		eventPeriod[event_index].period = period;
		event_index++;
...
//***IMPLEMENT THIS***
	   ...
	   (*semaPt) = (*semaPt) - 1;
				if ((*semaPt) < 0) {
						RunPt->blocked = semaPt; //reason why it's blocked
						EnableInterrupts();
						OS_Suspend();
				}
		 EnableInterrupts();
