...
//*FILL THIS IN****
	int32_t *blockPt;  // non-zero if blocked on this semaphore
	uint32_t sleep;    // non-zero if this thread is sleeping
...
// initialize TCB circular list
	 ...
	 tcbs[6].next = &tcbs[7];
	 tcbs[7].next = &tcbs[0];
// init thread stacks, including initial PC
...
// init sleep and blocked fields
   tcbs[0].blockPt = 0; tcbs[0].sleep = 0; tcbs[0].priority = 254;
	 tcbs[1].blockPt = 0; tcbs[1].sleep = 0; tcbs[1].priority = 254;
	 tcbs[2].blockPt = 0; tcbs[2].sleep = 0; tcbs[2].priority = 254;
	 tcbs[3].blockPt = 0; tcbs[3].sleep = 0; tcbs[3].priority = 254;
	 tcbs[4].blockPt = 0; tcbs[4].sleep = 0; tcbs[4].priority = 254;
	 tcbs[5].blockPt = 0; tcbs[5].sleep = 0; tcbs[5].priority = 254;
   tcbs[6].blockPt = 0; tcbs[6].sleep = 0; tcbs[6].priority = 254;
	 tcbs[7].blockPt = 0; tcbs[7].sleep = 0; tcbs[7].priority = 254;
// init RunPt
	 RunPt = &tcbs[0];
	 EndCritical(status);
  return 1;               // successful
...
// Same as Lab 3
	   ...
	   (*semaPt) = (*semaPt) - 1;
				if ((*semaPt) < 0) {
						RunPt->blockPt = semaPt; //reason why it's blocked
						EnableInterrupts();
						OS_Suspend();
		    }
  EnableInterrupts();  
