...
uint32_t Mail; //Shared Data for Mailbox
int32_t Send; //Semaphore for mailbox

void (*periodthr_task1Ptr)(void);
void (*periodthr_task2Ptr)(void);
int32_t period_task1;
int32_t period_task2;
int32_t Counter;
...
//	 status = StartCritical();
// initialize TCB circular list
	 periodthr_task1Ptr = thread1;
	 periodthr_task2Ptr = thread2;
	 period_task1 = period1;
	 period_task2 = period2;		
//	 EndCritical(status);
  //***YOU IMPLEMENT THIS FUNCTION*****
  return 1;
}
...
void Scheduler(void){ // every time slice
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	uint32_t LCM;
		LCM = period_task1 * period_task2;
		Counter = (Counter + 1)%LCM;
		if ((Counter % period_task1) == 0) {
					(*periodthr_task1Ptr)();
		}
		if ((Counter % period_task2) == 1) {
					(*periodthr_task2Ptr)();
		}
...
void OS_MailBox_Init(void){
  ...
  //***YOU IMPLEMENT THIS FUNCTION*****
	Mail = 0; //Shared Data for Mailbox
	OS_InitSemaphore(&Send, 0); //Semaphore for mailbox
}

// ******** OS_MailBox_Send******
// Enter data into the MailBox, dod ****** not spin/block if full
